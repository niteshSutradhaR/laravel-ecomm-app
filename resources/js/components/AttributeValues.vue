...
<script>
	export default {
		...
		methods:{
			...
			updateValue(){
				if (this.value==='') {
					this.$swal("Error, Value of attribute is required.",{
						 icon:'error',
					})
				} else {
					let attributeId=this.attributeid;
					...
					if (willdelete) {
						...
						axios.post('/admin/attributes/delete-values',{
							...
							if (response.data.status == 'success') {
								_this.values.splice(_this.key,1);
								_this.resetValue();
								_this.$swal('Success! Option value has been successfully deleted.',{
									icon:'success',
								});
							}else{
								_this.$swal("Your option value couldn't be deleted.");
							}
						});
					}
				});
